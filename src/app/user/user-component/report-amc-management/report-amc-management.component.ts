...
@Component({
  selector: 'app-report-amc-management',
  templateUrl: './report-amc-management.component.html',
  styleUrls: ['./report-amc-management.component.scss']
})
export class ReportAmcManagementComponent {
 empltyDataList: any[] = [];
  ...
  async getAMCdata() {
    ...
    try {
      ...
      const filteredResults = licenseReport.filter((item: any) => {
        ...
        return  isWarrantyValid && isPurchaseValid && item.item_name == 'AMC' 
      })
  ...
  exportToexcelfromnode(): any {
    ...
    const reportRequest = {
      reportTitle: "AMC Management Report",
      columns: [
        ...
        { header: 'AMC End Date', key: 'warrantyend_date', width: 30, format: 'date', filterButton: true },
    ...
    this.filesServices.exportToExcel(reportRequest).subscribe(
      (response: Blob) => {
        ...
        this.filesServices.downloadBlob(response, 'amc_management_report.xlsx');
      },
      (error) => {
